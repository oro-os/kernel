...
impl BitRange {
	...
	fn span(&self) -> Span {
		#[expect(clippy::option_if_let_else, reason = "more readable this way")]
...
impl TypeEx for Type {
	fn get_unsigned_primitive(&self) -> Option<String> {
		if let Self::Path(pth) = self
			&& let Some(segment) = pth.path.segments.last()
			&& (segment.ident == "u8"
				|| segment.ident == "u16"
				|| segment.ident == "u32"
				|| segment.ident == "u64"
...
#[expect(
	...
	reason = "highly complex proc macro; refactor in the form of a PR welcome"
)]
pub fn bitstruct(input: TokenStream) -> Result<TokenStream> {
	let Bitstruct {
		attributes,
		vis,
		name,
		...
		match &def.field_body {
			...
			FieldBody::ExtType(ext_ty) => {
				...
				match &ext_ty {
					...
					ExtType::As(as_conv) if as_conv.unsafe_.is_some() => {
						let ty = &as_conv.ty;
					...
					ExtType::From(from_conv) if from_conv.const_.is_some() => {
						let ty = &from_conv.ty;
			...
			FieldBody::Enum(enum_field) => {
				...
				#[expect(
					...
					reason = "improves readability in this case"
				)]
