...
impl<A: Arch> KernelInterface<A> for AddrLayoutV0<A> {
	fn get(&self, _thread: &Tab<Thread<A>>, index: u64, key: u64) -> InterfaceResponse {
		let segment = match index {
			k if k == key!("module") => AddressSpace::<A>::user_data(),
			k if k == key!("thrdstck") => AddressSpace::<A>::user_thread_stack(),
		...
		match key {
			k if k == key!("start") => InterfaceResponse::ok(segment.range().0 as u64),
			k if k == key!("end") => InterfaceResponse::ok(segment.range().1 as u64),
