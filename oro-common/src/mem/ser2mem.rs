	...
	P: PhysicalAddressTranslator,
{
	...
	unsafe fn reserve(&mut self, n: usize) -> Result<&'static mut [u8], MapError> {
		...
		if self.current_page_slice.is_empty() {
		...
		debug_assert!(!self.current_page_slice.is_empty());
		...
		let slice = core::slice::from_raw_parts_mut(
			self.current_page_slice.as_ptr().cast_mut(),
			byte_count,
		);

		self.current_page_slice = &self.current_page_slice[byte_count..];
		self.current_target_virt += byte_count;
