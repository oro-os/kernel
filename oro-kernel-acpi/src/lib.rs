...
pub use ::oro_kernel_acpica_sys as sys;
use oro_kernel_macro::assert;
use oro_kernel_mem::phys::{Phys, PhysAddr};
...
pub trait AcpiTable: Sized {
	...
	/// The underlying system table type from [`oro_kernel_acpica_sys`].
...
/// [`oro_kernel_acpica_sys`] table types and implements the higher level wrapper
/// and marker traits for them.
///
/// This macro itself is invoked by [`oro_kernel_acpica_sys::acpi_tablegen`], which
