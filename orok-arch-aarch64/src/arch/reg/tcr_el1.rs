...
bitstruct! {
	...
	pub struct TcrEl1(u64) {
		...
		///
		/// # Safety
		/// This field MUST _ONLY_ be set to a value no larger than that of [`crate::arch::reg::IdAa64Mmfr0El1::pa_size`] -
		/// the physical address size supported by the hardware. Setting it to a larger value can lead to undefined behavior.
		#[non_exhaustive]
		...
		#[expect(clippy::enum_variant_names, reason = "ARMARM parity")]
		#[non_exhaustive]
...
impl TcrEl1 {
	...
	///
	/// # Safety
	/// Improperly configuring the `TCR_EL1` register can lead to undefined behavior, since it
	/// controls the translation of virtual addresses to physical addresses. It should only be
	/// used to write valid configurations of the `TCR_EL1` register.
	#[orok_test::effect(write_reg = tcr_el1)]
...
impl TcrEl1 {
	...
	#[expect(
		...
		reason = "all arithmetic operations are OK here"
	)]
	pub const fn tt0_range(self) -> (usize, usize) {
		(0, (1 << (64 - self.t0sz())) - 1)
	}
	...
	pub const fn tt1_range(self) -> (usize, usize) {
		(
