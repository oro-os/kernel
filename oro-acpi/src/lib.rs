...
use oro_mem::translate::PhysicalAddressTranslator as Pat;
...
impl<P: Pat> Rsdp<P> {
	...
	/// [`oro_common::mem::translate::PhysicalAddressTranslator`].
	///
...
impl<P: Pat> core::fmt::Debug for Rsdp<P>
where
...
pub trait RootSdtSearch<P: Pat, const PTR_SIZE: usize>: AcpiTable<P>
where
...
impl<P: Pat> RootSdtSearch<P, 4> for Rsdt<P> {}
impl<P: Pat> RootSdtSearch<P, 8> for Xsdt<P> {}

impl<P: Pat> RootSdt<P> {
...
pub trait AcpiTable<P: Pat>: Sized {
	...
	/// Gets the [`oro_common::mem::translate::PhysicalAddressTranslator`]
...
macro_rules! impl_tables {
	...
	(@inner $ident:ident => ($systbl_ident:ty, $sig_ident:path, ( & [ u8; 5 ] ), $(#[doc = $doc:literal]),*)) => {
		...
		$(#[doc = $doc])*
		pub struct $ident<P: Pat> {
			ptr: &'static $systbl_ident,
			pat: P,
		}

		impl<P: Pat> AcpiTable<P> for $ident<P> {
		...
		// SAFETY(qix-): Uses unstable feature `trivial_bounds`.
		#[allow(trivial_bounds)]
		impl<P: Pat> core::fmt::Debug for $ident<P> where $systbl_ident: core::fmt::Debug {
			fn fmt(&self, f: &mut core::fmt::Formatter<'_>) -> core::fmt::Result {
				self.ptr.fmt(f)
			}
		}
