...
fn derive_struct(mut structure: ItemStruct) -> proc_macro::TokenStream {
	...
	for ref mut field in &mut structure.fields {
		...
		if let Some(nt) = new_type {
			...
			field_writes.push(quote! {
				target.#field_ident = ::oro_ser2mem::detail::serialize_iterator_to_slice(
	...
	quote! {
		const _: () = {
			use ::oro_ser2mem::detail::Serializable;

			#structure

			#[automatically_derived]
			unsafe impl #impl_generics ::oro_ser2mem::detail::Proxied for #orig_ident #ty_generics #where_clause {
			...
			#[automatically_derived]
			unsafe impl #impl_generics ::oro_ser2mem::detail::Serializable for #orig_ident #ty_generics #where_clause {
...
fn derive_enum(structure: ItemEnum) -> proc_macro::TokenStream {
	...
	quote! {
		#[automatically_derived]
		unsafe impl ::oro_ser2mem::detail::Serializable for #ident {
			type Target = Self;
...
/// `#[derive(Ser2Mem)]` macro for Oro boot protocol structures.
///
/// You shouldn't depend on this directly; use the
/// `oro-ser2mem` crate instead.
