...
pub unsafe fn initialize() -> Arc<dyn GetInstant> {
	...
	let Some(hpet) = crate::boot::protocol::find_acpi_table::<oro_kernel_acpi::Hpet>() else {
		panic!("no HPET found");
	};
...
const _: () = {
	oro_kernel_macro::assert::size_of::<HpetRegisters, 0x400>();
};
