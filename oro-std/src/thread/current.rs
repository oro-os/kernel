...
use ::oro::{id::iface::KERNEL_THREAD_V0, key, syscall_get, syscall_set};
...
pub fn current() -> Thread {
	...
	let id = syscall_get!(KERNEL_THREAD_V0, KERNEL_THREAD_V0, 0, key!("id"))
...
pub fn yield_now() {
	...
	// NOTE(qix-): which is totally valid but the kernel hasn't implemented
	// NOTE(qix-): TLS quite yet. So we do it (slowly) here each time.
	syscall_set!(KERNEL_THREAD_V0, KERNEL_THREAD_V0, 0, key!("yield"), 0)
